...
def parse(response_payload, balancer_manager):
    def _parse_max_members(value):
        m = route_used_pattern.match(value)
        if m:
            return(
                int(m.group(1)),
                int(m.group(2))
    ...
    if len(_bs_dt) >= 1:
        ...
        match = httpd_server_version_pattern.match(_bs_dt[0].text)
        if match:
        ...
        match = openssl_version_pattern.search(_bs_dt[0].text)
        if match:
    ...
    if len(_bs_dt) >= 2:
        ...
        match = httpd_server_build_date_pattern.match(_bs_dt[1].text)
        if match:
    ...
    for table in _bs_table_routes:
        for i, row in enumerate(table.find_all('tr')):
            ...
            route.traffic_to = TrafficData(value=cells[9].text, decoded=_decode_data_usage(cells[9].text))
            route.traffic_from = TrafficData(value=cells[10].text, decoded=_decode_data_usage(cells[10].text))
...
def _decode_data_usage(value):
    ...
    try:
        ...
        match = route_data_used_pattern.match(value)
        if match:
